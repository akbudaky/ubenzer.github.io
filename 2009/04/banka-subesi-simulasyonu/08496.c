...
struct musteri
{
    ...
    int gelis;//Müþterinin bankaya geldiði dakika.
    int bekleme_suresi;
};

struct gise
{
    int bos_mu;// Giþenin boþ olup olmadýðýný belirtir.
    int hizmet_sure;//Giþenin müþteriye vereceði hizmetin süresi
    struct musteri g_musteri;//Giþede hizmet gören müþteri.
    int top_musteri;//Giþeden hizmet almýþ toplam müþteri sayýsý.
    int top_sure;// Giþeden verilen hizmetlerin toplam süresi.
};
...
int main()
{
    ...
    while(finito==0)
    {
        /*Simulasyon süresi sona erdiðinde ayný dakika içerisinde önce
        eðer varsa giþelerde olan müþterilerin hizmet süreleri birer dakika azaltýlýr.
        Daha sonra eðer varsa giþelerde hizmeti biten müþteriler gönderilir.
        Eðer varsa kuyrukta bekleyen müþteriler eðer boþalmýþsa
        boþalan giþelere gönderilir.
        Hem kuyruk hem de giþeler boþsa döngüden çýkýlýr.*/
